...
define("WECOZA_CORE_VERSION", (defined("WP_DEBUG") && WP_DEBUG) ? "1.0.5-" . time() : "1.0.5");
...
spl_autoload_register(function (string $class) {
    ...
    $namespaces = [
        ...
        "WeCoza\\ShortcodeInspector\\" =>
            WECOZA_CORE_PATH . "src/ShortcodeInspector/",
...
add_action(
    ...
    */
        ...
        if (
            class_exists(\WeCoza\Learners\Controllers\LearnerController::class)
        ) {
        ...
        if (
            class_exists(\WeCoza\Classes\Controllers\ClassAjaxController::class)
        ) {
        ...
        if (
            class_exists(
                \WeCoza\Classes\Controllers\PublicHolidaysController::class,
            )
        ) {
        ...
        if (
            class_exists(\WeCoza\Events\Shortcodes\EventTasksShortcode::class)
        ) {
            \WeCoza\Events\Shortcodes\EventTasksShortcode::register();
        }
        if (
            class_exists(
                \WeCoza\Events\Shortcodes\MaterialTrackingShortcode::class,
            )
        ) {
        ...
        if (
            class_exists(
                \WeCoza\Events\Controllers\MaterialTrackingController::class,
            )
        ) {
        ...
        if (
            class_exists(\WeCoza\Clients\Controllers\ClientsController::class)
        ) {
            new \WeCoza\Clients\Controllers\ClientsController();
        }
        if (
            class_exists(\WeCoza\Clients\Controllers\LocationsController::class)
        ) {
        ...
        if (
            class_exists(
                \WeCoza\LookupTables\Controllers\LookupTableController::class,
            )
        ) {
        ...
        if (
            class_exists(
                \WeCoza\LookupTables\Ajax\LookupTableAjaxHandler::class,
            )
        ) {
        ...
        if (
            defined("WP_DEBUG") &&
            WP_DEBUG &&
            class_exists(\WeCoza\Dev\DevToolbarController::class)
        ) {
        ...
        add_action("wecoza_material_notifications_check", function () {
            try {
                if (
                    !class_exists(
                        \WeCoza\Events\Services\MaterialNotificationService::class,
                    )
                ) {
                    throw new \RuntimeException(
                        "MaterialNotificationService class not found",
                    );
                }
                ...
                if (!empty($orangeClasses)) {
                    $sentOrange = $service->sendMaterialNotifications(
                        $orangeClasses,
                        "orange",
                    );
                    if (defined("WP_DEBUG") && WP_DEBUG) {
                        error_log(
                            "WeCoza Material Cron: Sent {$sentOrange} orange (7-day) notifications",
                        );
                    }
                }
                ...
                if (!empty($redClasses)) {
                    $sentRed = $service->sendMaterialNotifications(
                        $redClasses,
                        "red",
                    );
                    if (defined("WP_DEBUG") && WP_DEBUG) {
                        error_log(
                            "WeCoza Material Cron: Sent {$sentRed} red (5-day) notifications",
                        );
                    }
                }
            } catch (\Exception $e) {
                wecoza_log(
                    "Material notification check failed: " . $e->getMessage(),
                    "error",
                );
                throw $e;
            }
        ...
        add_action("wecoza_process_notifications", function () {
            try {
                if (
                    !class_exists(
                        \WeCoza\Events\Services\NotificationProcessor::class,
                    )
                ) {
                    throw new \RuntimeException(
                        "NotificationProcessor class not found",
                    );
                }
        ...
        add_action(
            ...
            function (int $eventId) {
                try {
                    if (
                        !class_exists(
                            \WeCoza\Events\Services\NotificationEnricher::class,
                        )
                    ) {
                        throw new \RuntimeException(
                            "NotificationEnricher class not found",
                        );
                    }
                    ...
                    wecoza_log(
                        "Event processing failed for ID {$eventId}: " .
                            $e->getMessage(),
                        "error",
                    );
                    throw $e; // Re-throw for Action Scheduler retry
        ...
        add_action(
            ...
            function (
                ...
                try {
                    if (
                        !class_exists(
                            \WeCoza\Events\Services\NotificationEmailer::class,
                        )
                    ) {
                        throw new \RuntimeException(
                            "NotificationEmailer class not found",
                        );
                    }
                    ...
                    if (!$result["success"]) {
                        throw new \RuntimeException(
                            $result["error_message"] ?? "Email send failed",
                        );
                    }
                } catch (\Exception $e) {
                    wecoza_log(
    ...
    */
        ...
        add_action("wp_ajax_wecoza_mark_notification_viewed", function () {
            ...
            if (
                !class_exists(
                    \WeCoza\Events\Services\NotificationDashboardService::class,
                )
            ) {
                wp_send_json_error(
        ...
        add_action(
            "wp_ajax_wecoza_mark_notification_acknowledged",
            function () {
                check_ajax_referer("wecoza_notifications_nonce", "nonce");
...
register_activation_hook(__FILE__, function () {
    ...
    if (!extension_loaded("pdo_pgsql")) {
        ...
        wp_die(
            __(
                "WeCoza Core requires the PDO PostgreSQL extension (pdo_pgsql).",
                "wecoza-core",
            ),
            __("Plugin Activation Error", "wecoza-core"),
            ["back_link" => true],
    ...
    if (!wp_next_scheduled("wecoza_material_notifications_check")) {
        wp_schedule_event(
            time(),
            "daily",
            "wecoza_material_notifications_check",
        );
    }
...
register_deactivation_hook(__FILE__, function () {
    ...
    if ($notificationTimestamp) {
        wp_unschedule_event(
            ...
            "wecoza_process_notifications",
        );
    }

    // Unschedule legacy email notification cron (if any lingering)
    $legacyEmailTimestamp = wp_next_scheduled(
        "wecoza_email_notifications_process",
    );
...
add_action("admin_notices", function () {
    ...
    if (empty(get_option("wecoza_postgres_password", ""))) {
        ...
        if (
            $screen &&
            ($screen->id === "plugins" ||
                strpos($screen->id, "wecoza") !== false)
        ) {
            echo '<div class="notice notice-warning is-dismissible">';
            echo "<p><strong>" .
                esc_html__("WeCoza Core:", "wecoza-core") .
                "</strong> ";
            echo esc_html__(
                "PostgreSQL password not configured. Please set the wecoza_postgres_password option.",
                "wecoza-core",
            );
            echo "</p></div>";
        }
    ...
    WP_CLI::add_command("wecoza", function ($args, $assocArgs) {
        ...
        if (isset($args[0]) && $args[0] === "test-db") {
            ...
            if ($db->testConnection()) {
                ...
                WP_CLI::error(
                    "PostgreSQL connection failed. Check your credentials.",
                );
            }
            return;
        }
