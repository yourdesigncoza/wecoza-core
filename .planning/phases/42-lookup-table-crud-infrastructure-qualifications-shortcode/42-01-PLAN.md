...
must_haves:
  truths:
    - "Placing [wecoza_manage_qualifications] on a page outputs a Phoenix-styled card (no PHP errors, no blank output)"
    - "AJAX POST to wecoza_lookup_table with sub_action=list returns qualifications data as JSON"
    - "AJAX write operations (create/update/delete) are rejected without valid nonce + manage_options capability"
  ...
  artifacts:
    ...
    - path: "src/LookupTables/Controllers/LookupTableController.php"
      provides: "Shortcode registration + asset enqueuing + TABLES config constant"
  ...
  key_links:
    ...
    - from: "src/LookupTables/Ajax/LookupTableAjaxHandler.php"
      to: "src/LookupTables/Controllers/LookupTableController.php"
      via: "static call LookupTableController::getTableConfig()"
      pattern: "LookupTableController::getTableConfig"
    - from: "src/LookupTables/Ajax/LookupTableAjaxHandler.php"
...
- Table config is NOT stored here. Retrieve config via `LookupTableController::getTableConfig($tableKey)` (TABLES constant lives in Controller per CONTEXT.md architecture decision)
- Import: `use WeCoza\LookupTables\Controllers\LookupTableController;`
...
- If `LookupTableController::getTableConfig($tableKey)` returns null, send error "Invalid table." with 400
- If `sub_action` unknown, send error "Invalid action." with 400
  ...
  <verify>
    ...
    - grep confirms: `class LookupTableRepository`, `class LookupTableAjaxHandler`, `LookupTableController::getTableConfig`, `findAll`, `insert`, `update`, `delete`, `handleRequest`, `requireAuth`
  </verify>
  <done>
...
- Contains the TABLES constant (per CONTEXT.md architecture: "Table Configuration (in Controller)"):
...
- Public static accessor so AjaxHandler (and future consumers) can retrieve config:
  `public static function getTableConfig(string $key): ?array` — returns `self::TABLES[$key] ?? null`
...
- `renderManageTable(array|string $atts = [], string $content = '', string $tag = ''): string`:
  ...
  - Get config via `self::getTableConfig($tableKey)` (config is local to this class)
...
```
  ...
  <verify>
    ...
    - grep Controller confirms: `TABLES`, `getTableConfig`, `wecoza_manage_qualifications`, `wecoza_manage_placement_levels`, `enqueueAssets`, `renderManageTable`
    - grep AjaxHandler confirms: `LookupTableController::getTableConfig` (handler reads config from controller, not local constant)
  </verify>
  <done>
