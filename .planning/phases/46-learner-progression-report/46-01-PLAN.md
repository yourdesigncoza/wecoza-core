...
must_haves:
  truths:
    ...
    - "AJAX endpoint returns learner progressions filtered by employer/client company"
    ...
    - "Multi-learner results are grouped by client company name"
  artifacts:
...
1. `findForReport(array $filters = []): array` -- Fetches progression data for the report page. SQL is a 5-table JOIN: learner_lp_tracking + products + learners + classes + clients (via learners.employer_id = clients.client_id). Returns all progressions matching filters, ordered by client_name, learner surname, start_date DESC.
...
SELECT columns: lpt.*, p.product_name, p.product_duration, CONCAT(l.first_name, ' ', l.surname) AS learner_name, l.id AS learner_id, c.class_code, cl.client_name, cl.client_id AS employer_client_id

The JOIN for clients is: LEFT JOIN clients cl ON l.employer_id = cl.client_id (through learners table, NOT through classes).
...
grep -n "findForReport\|getReportSummaryStats" src/Learners/Repositories/LearnerProgressionRepository.php should show both new methods.
  ...
  <done>
Two new repository methods exist: findForReport returns filtered progression rows with learner/product/class/client JOINs, getReportSummaryStats returns aggregate counts and rates using the same filter criteria.
  </done>
</task>
...
5. Group the flat results by `employer_client_id` into a structure:
   ```php
   $grouped = [];
   foreach ($data as $row) {
       $clientId = $row['employer_client_id'] ?? 0;
       ...
       if (!isset($grouped[$clientId]['learners'][$learnerId])) {
           $grouped[$clientId]['learners'][$learnerId] = [
               'learner_id' => $learnerId,
               'learner_name' => $row['learner_name'],
               'progressions' => [],
           ];
       }
       $grouped[$clientId]['learners'][$learnerId]['progressions'][] = $row;
   }
...
grep -n "get_progression_report" src/Learners/Ajax/ProgressionAjaxHandlers.php should show both the handler function and the add_action registration.
  ...
  <done>
AJAX handler `handle_get_progression_report` exists, accepts search/employer_id/status filters via GET, returns grouped learner data by company plus summary stats, and is registered on the `get_progression_report` action.
  </done>
</task>
...
3. Response structure groups learners by client company
...
- Response includes `groups` (company-grouped) and `summary` (aggregate stats)
</success_criteria>
