...
8. assets/js/learners/progression-admin.js line 630: Change `data.hours_present` to `data.hours_trained` and update the label text from 'present' to 'trained':
  ...
  <verify>
    ...
    <manual>Result should be 0 — no remaining hours_present-based progress calculations or display numerators in these files</manual>
  </verify>
  <done>All SQL CASE expressions calculating progress_pct use hours_trained. All view templates show hours_trained as numerator. JS admin panel shows hours_trained. The hours_present column is still SELECTed where needed for other purposes but is not used for progress calculations.</done>
</task>
