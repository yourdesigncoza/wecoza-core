...
must_haves:
  ...
  key_links:
    ...
    - from: "views/agents/components/agent-capture-form.view.php"
      to: "views/agents/components/agent-fields.view.php"
      via: "wecoza_view include for field partials"
      pattern: "wecoza_view.*agent-fields"
---
...
- If the form includes agent-fields.php partial, replace:
  - OLD: `$this->load_template('agent-fields.php', $data, 'partials')`
  - NEW: `echo wecoza_view('agents/components/agent-fields', $data, true);`
  - NOTE: Use `echo wecoza_view(...)` since templates use echo, not return
...
Copy from `.integrate/wecoza-agents-plugin/templates/partials/agent-fields.php` with same transformations:
- ABSPATH guard
- Text domain change
- No template include calls needed (this is a leaf partial)
- Keep all FormHelpers references
- Keep all form field HTML

If the agent-fields.php partial does not exist as a separate file in the source (i.e., all fields are inline in agent-capture-form.php), then create a minimal agent-fields.view.php that contains a comment noting it's reserved for future extraction. But based on the file listing, `templates/partials/agent-fields.php` exists.
  </action>
  <verify>
```bash
