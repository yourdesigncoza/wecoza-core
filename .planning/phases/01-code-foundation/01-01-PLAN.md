...
<objective>
Establish Events module foundation in wecoza-core with PSR-4 autoloading and migrate core data layer classes.
...
- 10 migrated PHP classes with updated namespaces and database connection
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **Container.php special handling:**
    The Container class has factory methods that return repository/service instances. Update all internal namespace references from `WeCozaEvents\*` to `WeCoza\Events\*`. Factory methods will be updated in Plan 02 when Services are migrated.
  </action>
  <verify>
    ...
    - `grep "WeCozaEvents" src/Events/` returns no matches (all old namespaces replaced)
  </verify>
  <done>
    7 PHP files migrated with WeCoza\Events\* namespace. No references to WeCozaEvents namespace remain in migrated files.
  </done>
</task>
...
- [ ] 10 PHP files migrated (3 Models, 3 Repositories, 4 Support)
