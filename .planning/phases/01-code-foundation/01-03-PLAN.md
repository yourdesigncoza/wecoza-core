...
<task type="auto">
  ...
  <action>
    ...
    **IMPORTANT: Skip backup files.** Do NOT migrate files ending in `-bu.php` (e.g., `email-summary-bu.php`). These are backup files that should not be included in the migration.
  </action>
  <verify>
    ...
    - `ls views/events/*/*-bu.php 2>/dev/null && echo "FAIL: backup files migrated" || echo "PASS: no backup files"` confirms no backup files were migrated
  </verify>
  <done>
    9 view templates migrated to views/events/ directory structure. All namespace references updated from WeCozaEvents to WeCoza\Events. Backup files (*-bu.php) excluded from migration.
  </done>
</task>
...
4. **No backup files migrated:**
   ```bash
   ls views/events/*/*-bu.php 2>/dev/null && echo "FAIL: backup files present" || echo "PASS: no backup files"
   ```
...
6. **PHP syntax valid for all files:**
   ```bash
...
7. **Autoloader resolves all classes:**
   ```bash
...
- [ ] No backup files (*-bu.php) present in views/events/
</success_criteria>
