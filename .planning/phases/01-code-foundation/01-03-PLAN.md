...
- 10 View templates under `views/events/`
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **Controllers - apply standard transformations:**
    ...
    4. **WordPress exit check** at top of each file.
  ...
  <done>
    4 Controller classes and 3 Shortcode classes migrated with correct namespaces. All use statements updated to reference WeCoza\Events\* namespace.
  </done>
</task>
...
<task type="auto">
  <name>Task 3: Migrate Admin, CLI, and view templates</name>
  <files>
    src/Events/Admin/SettingsPage.php
    src/Events/CLI/AISummaryStatusCommand.php
    views/events/event-tasks/main.php
  ...
  <action>
    Migrate Admin settings, CLI command, and view templates:
    ...
    **View templates migration:**
    ...
    3. **Namespace updates in templates:**
    ...
    4. **Do NOT add `if (!defined('ABSPATH')) exit;`** to template files - they are included, not autoloaded.
  ...
  <verify>
    ...
    - `ls views/events/event-tasks/main.php views/events/ai-summary/main.php views/events/material-tracking/dashboard.php` confirms templates exist
    - `grep "WeCozaEvents" views/events/**/*.php` returns no matches (templates have updated namespaces)
    - `php -l src/Events/Admin/SettingsPage.php` returns no syntax errors
  </verify>
  <done>
    Admin SettingsPage and CLI command migrated. 9 view templates migrated to views/events/ directory structure. All namespace references updated.
  </done>
</task>
...
2. **No old namespace references anywhere:**
   ```bash
   grep -r "WeCozaEvents" src/Events/ views/events/ && echo "FAIL" || echo "PASS"
   ```
...
4. **PHP syntax valid for all files:**
   ```bash
   find src/Events -name "*.php" -exec php -l {} \; 2>&1 | grep -v "No syntax errors"
   find views/events -name "*.php" -exec php -l {} \; 2>&1 | grep -v "No syntax errors"
   ```

5. **Autoloader resolves all classes:**
   ```bash
