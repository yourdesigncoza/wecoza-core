...
<objective>
Manual browser testing of all CRUD operations, file uploads, statistics, working areas, and performance. Fix any bugs discovered.

Purpose: Verify end-to-end feature parity that automated CLI tests cannot cover (browser interactions, file uploads via $_FILES, visual badge rendering, Query Monitor performance checks).
...
<task type="auto">
  <name>Task 1: Pre-check debug log baseline and duplicate validation audit</name>
  <files></files>
  <action>
...
Read `AgentRepository::createAgent()` and check if the email/SA ID duplicate check properly excludes the current agent during updates. Specifically:
...
Report findings. If any code bugs are found, fix them immediately with atomic commits. Common issues to look for:
...
- Working area returning 0 instead of null
  ...
  <verify>
Code audit complete. Any bugs found are fixed and committed. Debug log baseline noted.
  </verify>
  <done>
All code paths audited for FEAT-01 through FEAT-05 correctness. Any bugs fixed with commits. Ready for manual browser verification.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Complete agents module with CRUD operations, file uploads, statistics badges, working areas dropdowns, and AJAX pagination/delete. All automated parity tests passed in Plan 01. Code audit completed in Task 1.
  </what-built>
  <how-to-verify>
...
12. Edit an agent, upload a PDF as signed agreement -- submit
13. Verify file link appears on agent view page
14. Try uploading a .jpg file -- should be rejected or ignored

**FEAT-04: Statistics Badges**

15. On the agents display page, verify 4 statistics badges visible:
...
16. Open agent capture form
17. Verify "Preferred Working Area" dropdowns populate with locations
18. Select areas for Preferred 1 and leave 2/3 empty
19. Submit -- should succeed without foreign key errors

**Performance (Bug #15 / #16)**

20. After any update, verify the display page shows updated data immediately (no stale cache)
21. If Query Monitor is installed: check for information_schema queries (should be minimal)

**Browser Console**

22. Open DevTools Console on each agent page
23. Verify 0 JavaScript errors
24. Check Network tab: all AJAX requests return 200
  </how-to-verify>
...
<verification>
- Code audit found and fixed any duplicate validation, statistics, or file upload bugs
- All 23 manual test steps verified by user
- No PHP errors in debug.log after testing
- No JS errors in browser console
- FEAT-01 through FEAT-05 requirements satisfied
</verification>

<success_criteria>
- Agent CRUD operations persist correctly to database
- Duplicate email/SA ID validation prevents duplicates
