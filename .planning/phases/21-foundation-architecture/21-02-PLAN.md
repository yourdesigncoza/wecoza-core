...
files_modified:
  ...
  - src/Clients/Shortcodes/ClientShortcodes.php
...
The source ClientsController has 10+ AJAX handler methods embedded. These should be REMOVED from the Controller and placed in the separate AJAX handler class (Task 2). Remove methods like:
...
5. Extract AJAX handlers to ClientAjaxHandlers (Task 2)
...
<task type="auto">
  <name>Task 2: Create AJAX handlers, migrate Views and JS, wire into wecoza-core.php</name>
  <files>
    src/Clients/Ajax/ClientAjaxHandlers.php
    views/clients/components/client-capture-form.view.php
    views/clients/components/client-update-form.view.php
...
Constructor: instantiate repositories + call `registerHandlers()`.
...
**2. Migrate View templates to `views/clients/`:**
...
**3. Copy JS files to `assets/js/clients/`:**
...
2. `find views/clients/ -name "*.php" -exec php -l {} \;` - All view files pass syntax check
3. `find views/clients/ -name "*.php" | wc -l` returns 6
4. `find assets/js/clients/ -name "*.js" | wc -l` returns 6
5. `grep -c "ClientsController" wecoza-core.php` returns >=1 (module initialized)
6. `grep -c "ClientAjaxHandlers" wecoza-core.php` returns >=1 (AJAX initialized)
7. `grep -c "manage_clients" wecoza-core.php` returns >=2 (add_cap + remove_cap)
8. `grep -r "WeCozaClients" views/clients/` returns 0 matches
9. `grep -r "WeCozaClients" src/Clients/Ajax/` returns 0 matches
10. `grep -c "AjaxSecurity" src/Clients/Ajax/ClientAjaxHandlers.php` returns >=3
  </verify>
  <done>
AJAX handlers use AjaxSecurity patterns. Views render from views/clients/ via wecoza_view(). JS loads from assets/js/clients/ via wp_enqueue_script. All module classes initialized in wecoza-core.php. Capabilities registered on activation.
  </done>
</task>
...
<verification>
After both tasks complete, verify the full Phase 21 success criteria:
...
- AJAX handlers use AjaxSecurity nonce/capability patterns
