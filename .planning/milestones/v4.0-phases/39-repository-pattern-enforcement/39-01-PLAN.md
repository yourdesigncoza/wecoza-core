...
- Per-repository method count table (repository name | total methods audited) to enable cross-check against actual codebase

**Verification step:** After writing the audit, run `grep -c 'function ' <file>` on each repository file and compare the method count to the audit table. Every public/protected method that executes SQL should be listed. Flag any discrepancies.
...
- LocationRepository::checkDuplicates -- JUSTIFIED (LOWER() case-insensitive matching)
  ...
  <verify>
File exists at .planning/phases/39-repository-pattern-enforcement/39-SQL-AUDIT.md. Contains a table for each of the 9 repositories. Summary section shows total, replaceable, justified counts, and per-repository method count table. Every method with direct SQL is listed. Run `grep -c 'function ' src/*/Repositories/*Repository.php` and compare counts against audit document -- no SQL-executing methods missing.
  </verify>
  <done>
Every direct SQL query across all 9 repository files is catalogued. Each has REPLACEABLE or JUSTIFIED classification with reason. Per-repository method counts cross-checked against codebase.
  </done>
</task>
