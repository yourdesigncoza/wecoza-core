...
@.planning/phases/33-agents-module-fixes/33-01-SUMMARY.md
@src/Agents/Controllers/AgentsController.php
@src/Agents/Ajax/AgentsAjaxHandlers.php
...
1. **`getAgentStatistics(): array`** — Move from AgentsController.php (the `getAgentStatistics` private method). Queries agents table for total, active, SACE registered, quantum qualified counts. Include the try/catch with error fallback returning zeros. Extract the empty statistics return into a private static `getEmptyStatistics()` helper method.

2. **`mapAgentFields(array $agent): array`** — Move from AgentsController.php (the `mapAgentFields` private method). Maps database column names to frontend display field names (agent_id->id, surname->last_name, tel_number->phone, email_address->email, etc).

3. **`mapSortColumn(string $column): string`** — Move from AgentsController.php (the `mapSortColumn` private method). Maps frontend sort column names to database column names (last_name->surname, phone->tel_number, email->email_address).

4. **`getDisplayColumns(string $columns_setting): array`** — Move from AgentsController.php (the `getDisplayColumns` private method). Returns array of column key=>label pairs, filtered by $columns_setting if provided.

Follow the existing service class conventions:
- `<?php` tag, namespace declaration, ABSPATH check
- PHPDoc class comment with `@package WeCoza\Agents` and `@since 3.1.0`
...
Verify all 4 public methods exist:
```bash
...
2. In `renderAgentsList()` method, replace private method calls with service calls:
   - `$sort_column = $this->mapSortColumn($sort_column);` -> `$sort_column = AgentDisplayService::mapSortColumn($sort_column);`
   - `$agents[] = $this->mapAgentFields($agent);` -> `$agents[] = AgentDisplayService::mapAgentFields($agent);`
   - `$statistics = $this->getAgentStatistics();` -> `$statistics = AgentDisplayService::getAgentStatistics();`
   - `$columns = $this->getDisplayColumns($atts['columns']);` -> `$columns = AgentDisplayService::getDisplayColumns($atts['columns']);`
...
2. In `handlePagination()` method, replace private method calls with service calls:
   - `$orderby = $this->mapSortColumn($orderby);` -> `$orderby = AgentDisplayService::mapSortColumn($orderby);`
   - `$agents[] = $this->mapAgentFields($agent);` -> `$agents[] = AgentDisplayService::mapAgentFields($agent);`
   - `$statistics = $this->getAgentStatistics();` -> `$statistics = AgentDisplayService::getAgentStatistics();`
   - `$columns = $this->getDisplayColumns('');` -> `$columns = AgentDisplayService::getDisplayColumns('');`
