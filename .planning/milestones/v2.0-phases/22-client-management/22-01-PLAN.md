...
must_haves:
  truths:
    - "User can access pages with Clients shortcodes without seeing error messages"
...
<task type="auto">
  ...
  <action>
    ...
    **Step 6: Verify all 3 CLIENT shortcodes render cleanly (Phase 22 success criteria)**
    After fixes, truncate debug.log, then re-run the 3 client shortcodes that are Phase 22 requirements:
    ```
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_capture_clients]');" | wc -c
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_display_clients]');" | wc -c
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_update_clients]');" | wc -c
    ```
    Each must produce HTML output > 0 chars. Then check debug.log for no new PHP Fatal/Warning errors from WeCoza\Clients namespace.

    **Step 6b: (BONUS smoke test — NOT Phase 22 success criteria)**
    The 3 location shortcodes belong to Phase 23 (Location Management). Test them opportunistically
    as a smoke test only — failures here do NOT block Phase 22 completion:
    ```
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_locations_capture]');" | wc -c
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_locations_list]');" | wc -c
    wp eval "wp_set_current_user(1); echo do_shortcode('[wecoza_locations_edit]');" | wc -c
    ```
    Log any location shortcode errors for Phase 23 but do not block on them.
  ...
  <done>
    All 3 client shortcodes (wecoza_capture_clients, wecoza_display_clients, wecoza_update_clients) render HTML output via WP-CLI do_shortcode without PHP fatal errors or warnings. Debug log clean of Clients module errors. Location shortcodes (wecoza_locations_capture, wecoza_locations_list, wecoza_locations_edit) smoke-tested as bonus — failures logged for Phase 23 but do not block Phase 22.
  </done>
</task>
