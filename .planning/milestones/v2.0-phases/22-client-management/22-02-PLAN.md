...
<task type="auto">
  ...
  <action>
    ...
    **Step 2: Test CSV export (wecoza_export_clients) — model AND handler**

    **Step 2a: Verify model-level CSV data generation**
    - Invoke model directly via WP-CLI to confirm data retrieval works:
      ```
      wp eval "
        wp_set_current_user(1);
    ...
    **Step 2b: Verify the actual AJAX handler (ClientAjaxHandlers::exportClients())**
    - Read the source of `src/Clients/Ajax/ClientAjaxHandlers.php` and locate the `exportClients()` method.
    - Verify source code contains:
      - `Content-Type: text/csv` header (or `application/csv`)
      - `Content-Disposition: attachment` header with `.csv` filename
    - If headers missing or incorrect, fix them in-place.
    - Test the full handler via WP-CLI output capture:
      ```
      wp eval "
        wp_set_current_user(1);
        \$_POST = ['action' => 'wecoza_export_clients', 'nonce' => wp_create_nonce('clients_nonce_action')];
        \$_REQUEST = \$_POST;
        ob_start();
        do_action('wp_ajax_wecoza_export_clients');
        \$output = ob_get_clean();
        echo 'CSV handler output: ' . strlen(\$output) . ' bytes' . PHP_EOL;
        if (strlen(\$output) > 0) {
          \$lines = explode(PHP_EOL, trim(\$output));
          echo 'CSV header row: ' . (\$lines[0] ?? 'EMPTY') . PHP_EOL;
          echo 'Total lines: ' . count(\$lines);
        }
      "
      ```
    - Verify handler produces CSV output > 0 bytes with correct header row
    - Check debug.log for errors after handler invocation

    **IMPORTANT per user decisions:**
    - CSV export format and contents preserved as-is — do not alter column order, headers, or data formatting
  ...
  <verify>
    ...
    2. CSV export (model): WP-CLI invocation produces CSV data with correct columns and > 0 rows
    3. CSV export (handler): do_action('wp_ajax_wecoza_export_clients') produces CSV output; source code contains Content-Type: text/csv and Content-Disposition: attachment headers
    4. Statistics: getStatistics() returns array with expected keys, excludes deleted clients
    5. Sites: head/sub site CRUD works, hierarchy returns tree structure
    6. Locations: get/save/duplicate-check endpoints respond correctly
    7. JS action names match PHP handler names across all `assets/js/clients/*.js` files (grep confirms)
    8. No PHP errors in debug.log after all tests
  </verify>
  <done>
    All secondary AJAX endpoints (hierarchy, export, statistics, sites, locations) respond correctly. CSV export verified at both model level (data retrieval) and AJAX handler level (do_action output + Content-Type/Content-Disposition headers in source). Statistics return correct counts excluding deleted clients. JS files reference correct AJAX action names for all endpoints. Debug log clean.
  </done>
</task>
...
6. CSV export: model produces valid CSV data AND AJAX handler (do_action) outputs CSV with correct headers (Task 1B)
