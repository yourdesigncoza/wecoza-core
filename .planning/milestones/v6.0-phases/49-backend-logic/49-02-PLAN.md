...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/Learners/Repositories/LearnerProgressionRepository.php"
      provides: "deleteHoursLogBySessionId method for session-based hours log cleanup"
      contains: "deleteHoursLogBySessionId"
  key_links:
    ...
    - from: "src/Classes/Services/AttendanceService.php"
      ...
      via: "repository method call for hours log deletion"
      pattern: "deleteHoursLogBySessionId"
---
...
3. **CRITICAL FORMAT MAPPING:** The DB stores timeData with `perDayTimes` containing `start_time`/`end_time` keys, but `ScheduleService::getTimesForDay()` expects `perDay` with `startTime`/`endTime`. Before calling generateScheduleEntries, transform the timeData:
   - If `timeData['perDayTimes']` exists, convert to `timeData['perDay']` format:
     ...
     ```
   - **After the loop, set the mode flag so `getTimesForDay()` enters the per-day branch:**
     ```php
     $timeData['mode'] = 'per-day';
     ```
   - This handles both old `start_time` and new `startTime` key formats in the DB, AND sets the mode that `getTimesForDay()` checks at line 183 of ScheduleService.
...
Follow established patterns: `declare(strict_types=1)`, ABSPATH guard, use statements for all dependencies, try/catch with `wecoza_sanitize_exception()`.
  ...
  <verify>
    <automated>cd /opt/lampp/htdocs/wecoza/wp-content/plugins/wecoza-core && php -l src/Classes/Services/AttendanceService.php && echo "--- Method presence check ---" && grep -c "generateSessionList\|validateSessionDate\|generateScheduleEntries\|perDayTimes\|mode.*per-day" src/Classes/Services/AttendanceService.php</automated>
    <manual>Verify: generateSessionList reads schedule_data and delegates to ScheduleService, validateSessionDate checks date membership, perDayTimes -> perDay format mapping present, mode = 'per-day' is set after conversion</manual>
  </verify>
  <done>AttendanceService has generateSessionList() that correctly maps DB schedule format to ScheduleService input (including setting mode='per-day') and merges existing session data, and validateSessionDate() that rejects non-scheduled dates. BACK-05 and BACK-06 addressed.</done>
</task>

<task type="auto">
  <name>Task 2: Add captureAttendance, markException, deleteAndReverseHours methods and repository helper</name>
  <files>src/Classes/Services/AttendanceService.php, src/Learners/Repositories/LearnerProgressionRepository.php</files>
  <action>
Add three more public methods to AttendanceService (in the same file created in Task 1). Also add `deleteHoursLogBySessionId()` to `LearnerProgressionRepository`.
...
5. For each learner in $learnerHours, call (note: use `$learner` as the loop variable to avoid ambiguity with the outer `$learnerHours` array):
   ```php
   foreach ($learnerHours as $learner) {
       $this->progressionService->logHours(
           ...
           hoursPresent: $learner['hours_present'],
           source:       'attendance',
   ...
   ```
   Wrap each logHours call in try/catch inside the loop. If it fails (learner has no in-progress LP), collect the error but continue with other learners. Do NOT abort the whole capture.
...
3. If session status is 'captured':
   ...
   c. For each learner that had hours logged, recalculate their LP accumulators by calling `$this->progressionService->recalculateHours($trackingId)`.
      ...
      - Delegate to `LearnerProgressionRepository::deleteHoursLogBySessionId(int $sessionId): array` (added in this plan — see note below). This method:
        ```sql
      ...
      - **NOTE:** Add `deleteHoursLogBySessionId()` as a new public method on `LearnerProgressionRepository` (which already operates on `learner_hours_log` via `logHours()` and `getHoursLog()`). This keeps all hours-log DB access in the repository layer. Inject `LearnerProgressionRepository` into AttendanceService constructor alongside `ProgressionService` (or access via `$this->progressionService` if it exposes the repo).
...
**LearnerProgressionRepository addition:**
Add the following public method to `src/Learners/Repositories/LearnerProgressionRepository.php` (alongside existing `logHours()` and `getHoursLog()` methods that already operate on `learner_hours_log`):
...
Then in `AttendanceService` constructor, inject `LearnerProgressionRepository` as `$this->progressionRepo` and call `$this->progressionRepo->deleteHoursLogBySessionId($sessionId)` instead of raw SQL in `deleteAndReverseHours()`.
  </action>
  <verify>
    <automated>cd /opt/lampp/htdocs/wecoza/wp-content/plugins/wecoza-core && php -l src/Classes/Services/AttendanceService.php && php -l src/Learners/Repositories/LearnerProgressionRepository.php && echo "--- Service method check ---" && grep -c "captureAttendance\|markException\|deleteAndReverseHours\|logHours\|recalculateHours" src/Classes/Services/AttendanceService.php && echo "--- Repo method check ---" && grep -c "deleteHoursLogBySessionId" src/Learners/Repositories/LearnerProgressionRepository.php</automated>
    <manual>Verify: captureAttendance uses $learner loop variable (not $learnerHours), markException creates zero-hours session, deleteAndReverseHours delegates log deletion to LearnerProgressionRepository::deleteHoursLogBySessionId()</manual>
  </verify>
  <done>AttendanceService has five public methods covering all attendance business logic. captureAttendance propagates hours to learner_lp_tracking via logHours with correct foreach variable naming. markException creates zero-hours sessions. deleteAndReverseHours reverses accumulated hours via LearnerProgressionRepository::deleteHoursLogBySessionId() (repository layer, not raw SQL in service). LearnerProgressionRepository has new deleteHoursLogBySessionId() method. SESS-01, SESS-02, SESS-04, SESS-05 addressed.</done>
</task>
